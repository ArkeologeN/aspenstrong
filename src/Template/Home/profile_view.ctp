...
//echo "<pre>";print_r($asConnection);die;
?>
...
<?php 
  if( ( $asConnection['close_status'] == "deactivated" || $asConnection['close_status'] == "deleted" ) ) {
    if(!empty($userSession) && $userSession->ID == $asConnection->owner)
    {
      echo '<p style="text-align: center; padding: 1%;color: red;">Your account is Deactivated.</p>';
    }else
    {
      echo '<p style="text-align: center; padding: 1%;color: red;">The User no longer exists.</p>';
    }
  }elseif($asConnection['status'] == "unapproved" && ( empty($asConnection->as_connections_meta) || (empty($asConnection->as_connections_meta->malpractice_insurance) || empty($asConnection->as_connections_meta->personal_statement)) || (empty($asConnection->as_connections_meta->issue) || empty($asConnection->as_connections_meta->age_group)) || (empty($asConnection->as_connections_emails) || empty($asConnection->as_connections_phones) || empty($asConnection->as_connections_addresses)))) 
  {
    if(!empty($userSession) && $userSession->ID == $asConnection->owner)
    {
  ...
  }elseif($asConnection['status'] == "unapproved" && $asConnection['close_status'] == "activated"){
  if(!empty($userSession) && $userSession->ID == $asConnection->owner)
    {?>
...
<?php 
  ...
  if($sessionuser==$id && ($asConnection['close_status'] != "deactivated" || $asConnection['close_status'] != "deleted")){
    if(!empty($userSession) && $userSession->ID == $asConnection->owner)
    {
  ?>
  <p style="text-align: center; padding: 1%;color: red;">Changes to profile needs admin approval.</p>
<?php }} }
...
?>
         ...
         <div class="profile_bg">
            	...
            	<div class="bg_overlay">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12"> 
                    	<div class="profile_display2">
                         ...
                         $final_add="";foreach($asConnection['as_connections_addresses'] as $address){
								   $final_add=$address['address'];
								   if($address['preferred']==1 && $address['type']=='Site'){?>
                                   <p> <?php echo $address['address'].' , '.$address['city'].' , '.$address['zipcode'];?> </p>
                                   <?php }}?>
                         
                         <?php usort($asConnection['as_connections_addresses'],"cmp");$z=1;foreach($asConnection['as_connections_addresses'] as $address){
            ...
            <div class="container">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 space35">
                        	<div class="descrptn">
                               ...
                               <p><?php echo $asConnection['as_connections_meta']['personal_statement'];?></p>
								   ...
								   ?>
										<li> <span> Business Statement (<?php echo $lang; ?>)</span> : <?php if(isset($other_statement[$key])){ echo $other_statement[$key]; }?> </li>
								   ...
								   <?php  } } ?>
								    <?php if($asConnection['entry_type']=='organization'){?>
									   ...
									   <li> <span> Accreditation </span> : <?php if(isset($asConnection['as_connections_meta']['accredatation'])){ echo $asConnection['as_connections_meta']['accredatation'];}?> </li>
                                        <?php if(isset($asConnection['as_connections_meta']['accredatation']) && $asConnection['as_connections_meta']['accredatation']=="Yes"){echo '<li><span> Accreditation Name </span> :'.$asConnection['as_connections_meta']['accredatation_name'].'</li>';}?>
                                   </ul>
                                   <?php }?>
                              </div>     
                            </div>     
             ...
             <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 space35">
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 space35">
                <?php  
                if($asConnection['as_connections_meta']['diagnosis'] !=""){
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 space35">
            		<div class="">
                   <?php 
                   if($asConnection['entry_type']=='individual'){
               ...
               $register_year = date('Y', strtotime($userDeteReg));
               $current_year = date('Y');
               $exp = $current_year - $register_year;
					   ?>
                   <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                         <div class="layout_profile">
                        	 <h3> Qualifications </h3>
                             <ul>
                               <li> Years in Practice : <?php if($asConnection['as_connections_meta']['practice']!=""){echo $practice_year_new;}else{echo 0;}?> Years </li> 
                     ...
                     <?php if (strpos($asConnection['as_connections_meta']['insurance_provider'], '|') !== false) 
                         ...
                         <div class="layout_profile layout_profile_purple">
                             ...
                             <ul>
                                ...
                                <li> Accepted Insurance Plans : <?php if(!empty($newvals_IP) && $asConnection['as_connections_meta']['insurance']=='Yes'){ echo $newvals_IP; }else{ echo 'NA';}?> </li> 
                                ...
                                <?php } ?>
                                <li> Accept Video Conference : <?php echo $asConnection['as_connections_meta']['video_conference'];?> </li>
                                <li> <?php if($asConnection['as_connections_meta']['declaration']==1){ echo 'Provider can provide a Superbill with CPT & ICD codes. ';}else{echo "Provider can't provide a Superbill with CPT & ICD codes. ";}?> </li>
                      ...
                      <?=$this->Flash->render();?>	
                         <h3>Contact Provider</h3>
                           <form name="contact_form" method="post" action="" id="contactform">
                               ...
                               <div class="user_display_form_vals" style="width: auto;  margin: 0 auto; display: inline-block;">
                                  <?php /*?><!-- <img src="captcha.php"> -->
                                   ...
                                   <span class="captcha"><?php echo $code;?></span>
                                    <input placeholder="Your captcha" type="text" name="captcha" style="width:15%; margin-left:10px;" required="required"><?php */?>
  ...
  <script src="https://maps.google.com/maps/api/js?key=AIzaSyAn8MyXKfr-KMBInEgmJfESScGLhk2kY-E&callback=initMap" type="text/javascript"></script>
    ...
    function initialize() {
	   ...
	   var address='<?php echo $asConnection['as_connections_addresses'][1]['address'].', '.$asConnection['as_connections_addresses'][1]['city'].', '.$asConnection['as_connections_addresses'][1]['state'].', '.$asConnection['as_connections_addresses'][1]['zipcode']; ?>';
