...

{
...

    {
	 ...
	 $therapiests=$this->Therapists->find('all',array('conditions'=>array('status'=>'a'),'order' => array('name' => 'ASC')));                 
	 //->where(['status ' => 'a']);
	...
	$this->viewBuilder()->setLayout("front");
	   ...
	   $modalities=$this->Modalities->find('all',['order' => ['name' => 'ASC']])                   
		...
		$financialSupport=$this->FinancialSupport->find('all',['order' => ['name' => 'ASC']])                   
		->where(['status ' => 'a']);
		
		$this->loadModel('LanguageOptions');
		$languageOptions=$this->LanguageOptions->find('all',['order' => ['name' => 'ASC']])                   
		->where(['status ' => 'a']);
		...
		if(isset($_POST['search_page']) && $_POST['search_page']==1){
			...
			$results = $this->AsConnections->find('all',['conditions'=>$conditions,'contain' => ['AsConnectionsSocials','AsConnectionsLinks','AsConnectionsAddresses','AsConnectionsMetas'],'order'=> ['AsConnections.id' => 'DESC']])->where(['status ' => 'approved']);	
				}else{
			$results = $this->AsConnections->find('all',['conditions'=>$conditions,'contain' => ['AsConnectionsSocials','AsConnectionsLinks','AsConnectionsAddresses','AsConnectionsMetas'],'order'=> ['AsConnections.first_name' => 'ASC']])->where(['status ' => 'approved']);
		...
		$cond1=$_POST['location'];			$conditionsnew['AsConnectionsAddresses.zipcode']=$_POST['location'];
		}else{
		$cond2=$_POST['location'];			$conditionsnew['AsConnectionsAddresses.zipcode']=$_POST['location'];
			}
			}
		...
		if(isset($_REQUEST['sort_id']) && $_REQUEST['sort_id']==1){
			$results = $this->AsConnections->find('all',['conditions'=>$conditions,'contain' => ['AsConnectionsSocials','AsConnectionsLinks','AsConnectionsAddresses'=> array('conditions' => $conditionsnew),'AsConnectionsMetas'],'order'=> ['AsConnections.id' => 'DESC']])->where(['status ' => 'approved']);	
				}else{
		$results = $this->AsConnections->find('all',['conditions'=>$conditions,'contain' => ['AsConnectionsSocials','AsConnectionsLinks','AsConnectionsAddresses'=> array('conditions' => $conditionsnew),'AsConnectionsMetas'],'order'=> ['AsConnections.first_name' => 'ASC']])->where(['status ' => 'approved']);
				}
		...
		$results = $this->AsConnections->find('all',['contain' => ['AsConnectionsSocials','AsConnectionsLinks','AsConnectionsMetas','AsConnectionsAddresses'],'order'=> ['AsConnections.first_name' => 'ASC']])->where(['status ' => 'approved']);
				}
	  ...
	  $Therapist_categories= $Therapist_category->find('all', array('order'=>array('name'=>'asc')));
 ...
 $asConnection_new = $this->AsConnections->get($id,['contain' => ['AsConnectionsEmails']]);
	...
	$vars = array(
			...
			if($this->sendMail($template_name,$from_email,'Aspen Strong Directory',$asConnection_new['as_connections_emails'][0]['address'],
			$data['name'],$vars,$subject)){
			$this->Flash->success_contact('Message sent successfully.');
			}
	...
	$this->Flash->success_contact('Please enter correct captch code.');	
		}
	 }
	$this->viewBuilder()->setLayout("front");
   ...
   if($record3->close_status == 'deleted')
   {
	...
	$record2 = $con_metas->find()->select(['practice','practice_year'])->where(['entry_id' => $id])->first();
	...
	$asConnection = $this->AsConnections->get($id,['contain' => ['ConnectionCredentials','AsConnectionsSocials','AsConnectionsLinks','AsConnectionsAddresses','AsConnectionsPhones','AsConnectionsEmails','AsConnectionsMetas']]);
	 ...
	 $asConnection['program_status']=$newVals5['program_status'];
	 	...
	 	$userDetails = $this->Users->get($asConnection->owner);
	 	$userDeteReg = $userDetails->user_registered->format('Y-m-d');
